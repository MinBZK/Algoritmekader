...
def on_env(env, config: MkDocsConfig, files: Files):
    ...
    for file in files:
        ...
        file.page.content = re.sub(
            r"<!-- list (.*?) -->", replace, file.page.content, flags=re.I | re.M
        )
...
def _create_table_row(file: File):
    ...
    return "".join(
        [
            ...
            f"<td>{_icon_true() if 'niet-impactvol' in toepassingen else _icon_false()}</td>",
            f"<td>{_icon_true() if 'impactvol' in toepassingen else _icon_false()}</td>",
            f"<td>{_icon_true() if 'hoog-risico' in toepassingen else _icon_false()}</td>",
            "</tr>",
        ]
    )
